...
1. **Equal option length** — all four options MUST be roughly the same length (similar word and
   character count). The correct option must never be noticeably longer/shorter; length must never
   hint at the answer.
2. **Material-only vocabulary** — every word in the question, ALL four options (INCLUDING the wrong
   ones), and the explanation MUST come from the provided session / material / PPT. Do NOT introduce
   any term, tool, technology, or concept that is not present in the provided material — not even in
   distractors. Build wrong options from real terms in the material, recombined incorrectly.
3. **Short explanation** — keep each explanation to 1–2 lines (one or two sentences) maximum.
4. **Generic & clean questions** — do NOT build questions from use-cases, scenarios, stories, or
   analogies. Ask directly and generically about the concept/term itself (e.g. "What is X?",
   "Which statement about X is correct?") — never "Imagine a team that…" or analogy-based framings.
...
4. Wrong options must use ONLY words/terminology found in the material (see strict rule 2) and match the other options in length (strict rule 1).
...
#### Format (FOLLOW EXACTLY — the layout below is what the UI renders):

The QUESTION_TEXT MUST be laid out as TWO clearly-labelled columns, each item on its OWN line,
with a single blank line between the two columns. Column A is numbered (1, 2, 3); Column B is
lettered (A, B, C). Do NOT put the columns on one line or use commas to separate items.

```
QUESTION_TEXT: Match each item in Column A with the correct item in Column B:
...
#### Options Format (each option is a COMPLETE one-to-one mapping):
...
Rules: every option must map EVERY number in Column A to exactly one letter in Column B (no item
left out, no letter reused). Keep Column A items and Column B descriptions similar in length.
...
### QUESTION CREATION CHECKPOINTS (every question MUST satisfy all)

1. **Syllabus & material alignment** — the question is covered by, and strictly based on, the provided reading material. Never ask about anything not taught in it.
2. **Keyword alignment** — use the topic's own terminology from the material.
3. **Bloom's taxonomy** — target an appropriate cognitive level and vary levels across the set (not all recall); set BLOOM_LEVEL accordingly.
4. **Clarity & readability** — simple and clear on the first read; one idea per question; no ambiguity.
5. **Don't reveal the answer** — the question stem must NOT contain or restate the correct option.
6. **Don't test the untaught** — never require knowledge outside the provided material.
7. **Technical accuracy** — the question, options, correct answer(s), and explanation must all be technically correct.
